...
String comb; 
	...
	@Override
	public List<Integer> getBusaDestinoApto(Integer distance,String calle1,String calle2){
		try{
		
		String query=	"select distinct c.idgeom from apartamento c, aptogeom g, paradas p where c.idgeom=g.id and ST_Intersects(ST_Buffer(g.punto,"+distance+"),ST_Transform(p.geom,32721) ) and p.desc_linea in" +
			...
			" (select distinct linea from (select ST_Intersection(buff.geom,bus.geom) as result ,bus.desc_linea as linea from (select distinct ST_Buffer(e,200) as geom"+
					" from (select (ST_Intersection(ca.geom,ca2.geom)) as e "+
						" from (select *"+ 
							" from ejes1 c where c.nom_calle='"+calle1+"') ca,"+ 
							" (select * from ejes1 c where c.nom_calle='"+calle2+"') ca2  )inter  "+
		...
		for(Integer i:result){
			...
			System.out.println("GET DESTINO DE BUS ap"+i);
		}
		return result;
...
public void modificarApto(Apartamento apart) {
	...
	try {
		em.merge(apart);			
		em.createNativeQuery("update aptogeom g set estado ='"+apart.getEstado()+"'  where g.id='"+apart.getIdGeom()+"'").executeUpdate();
	} catch (Exception e) {
